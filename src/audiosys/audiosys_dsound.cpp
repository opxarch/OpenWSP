    ...
    int init(int rate, int channels, int format, int flags) {
        ...
        switch(format){
            ...
            case AF_FORMAT_S8:
                break;
            default:
                ws_log(log::M_AUDIOSYS, log::VERBOSE)
                    << "ao_dsound: format " << af_fmt2str_short(format) << " not supported defaulting to Signed 16-bit Little-Endian."
                    << endlog;
        ...
        if(m_buffersize==-1) m_buffersize = m_bps; // space for 1 sec
        ...
        if ( res != DS_OK ) {
            ws_log(log::M_AUDIOSYS, log::WARN)
...
private:
    ...
    */
    ...
    {
      ws_log(log::M_AUDIOSYS, log::FATAL)
               << "\n-ao dsound commandline help:\n"
               << "Example: -ao dsound:device=1\n"
               << "  sets 1st device\n"
               << "\nOptions:\n"
...
static BOOL CALLBACK DirectSoundEnum(LPGUID guid,LPCSTR desc,LPCSTR module,LPVOID context) {
    ...
    ws_log(log::M_AUDIOSYS, log::VERBOSE) << endlog;
    (*device_index)++;
    return TRUE;
}
