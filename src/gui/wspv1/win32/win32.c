...
static DWORD     _hWndThread;

/* class names */
static const char acClassNameLCDInfo[]   = "Viewer LCD Info";
static const char acClassNameLCD[]       = "Viewer LCD";
static const char acClassNameDevice[]    = "Viewer Device";
static const char acClassNameHardkey[]   = "Viewer Hardkey";
#if DEBUG_GUI_LOG
static const char acClassNameLog[]       = "ViewerLog";
#endif // DEBUG_GUI_LOG
...
static void _MessageBox1(LPCTSTR lpText) {
  MessageBox(_hWndMain, lpText, MSGBOX_TITLE, MB_OK | MB_ICONEXCLAMATION);
}
...
static void _CreateWndLCD(void) {
  ...
  for (i = 0; i < NumDisplays; i++) {
    ...
    _hWndMain = CreateWindow(acClassNameLCD, acTitle,
                        WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME
                        | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_CLIPSIBLINGS,
                        CW_USEDEFAULT, CW_USEDEFAULT, XSize * _MagX + 2 * xSizeFrame, YSize * _MagY + ySizeCaption + 2 * ySizeFrame,
                        NULL, NULL, _hInst, NULL);
    SetWindowLong(_hWndMain, GWL_USERDATA, i);
...
*       _WinMain_NoClean
*/
static int _WinMain_NoClean(void) {
  char *sErr;
  ...
  CreateThread(NULL,0,wndThread,hInstance,0,&_hWndThread);
