...
#include "stheader.h"
#include "genres.h"
#include <openwsp/intreadwrite.h>
        ...
        if (((*list)->next_frame_pos < st_pos) || ((*list)->mp3_chans != mp3_chans)
          ...
          *list = tmp;
          ...
          if ((*list)->cons_hdrs >= MIN_MP3_HDRS) {
            ...
            tmp = (mp3_hdr_t*)malloc(sizeof(mp3_hdr_t));
...
static void get_flac_metadata (demuxer_t* demuxer)
{
    uint8_t preamble[4];
    ...
    do {
        ...
        ret = stream_read (stream, (char *) preamble, FLAC_SIGNATURE_SIZE);
        if (ret != FLAC_SIGNATURE_SIZE) return;
        ...
        switch (preamble[0] & 0x7F) {
            case FLAC_STREAMINFO: {
                ...
                stream_skip (stream, FLAC_STREAMINFO_SIZE);
                break;
            }

            case FLAC_PADDING:
                stream_skip (stream, blk_len);
                break;
            ...
            case FLAC_SEEKTABLE: {
                ...
                for (i = 0; i < seekpoint_count; i++)
                    if (stream_skip (stream, FLAC_SEEKPOINT_SIZE) != 1)
                        return;
                break;
            }
            ...
            case FLAC_VORBIS_COMMENT: {
                ...
                uint8_t *ptr = (uint8_t*)comments;
                char *comment;
                int cn;
                char c;

                //FIXME?
                if ((unsigned int)stream_read (stream, comments, blk_len) == blk_len) {
                    length = AV_RL32(ptr);
                    ptr += 4 + length;
                    ...
                    cn = 0;
                    //FIXME?
                    for (; (uint32_t)cn < comment_list_len; cn++) {
                        length = AV_RL32(ptr);
                        ptr += 4;
...
static int demux_audio_open(demuxer_t* demuxer)
{
    ...
    stream_read(s, (char*)hdr, HDR_SIZE);
    while(n < 30000 && !s->eof) {
        ...
        if( hdr[0] == 'R' && hdr[1] == 'I' && hdr[2] == 'F' && hdr[3] == 'F' ) {
            ...
            if(hdr[0] != 'W' || hdr[1] != 'A' || hdr[2] != 'V'  || hdr[3] != 'E' )
                ...
                frmt = WAV;
                break;
            } else if((mp3_flen = getmp3Header(hdr, &mp3_chans, &mp3_freq,
            ...
            stream_read(s, (char*)&hdr[HDR_SIZE - step], step);
            n++;
    }
    ...
    switch(frmt) {
        ...
        case WAV: {
            ...
            len = stream_read_dword_le(s);
            if(len < 16) {
                ws_log(log::M_DEMUXER, log::ERR)
                    << "[demux_audio] Bad wav header length: too short ("
                    << len << ")!!!"
                    << endlog;

                len = 16;
            }
            if(len > MAX_WAVHDR_LEN) {
                ws_log(log::M_DEMUXER, log::ERR)
                    << "[demux_audio] Bad wav header length: too long ("
                    << len << ")!!!"
                    << endlog;

                len = 16;
            }
            sh_audio->wf = w = (WAVEFORMATEX*)malloc(/*FIXME?*/(unsigned)len > sizeof(WAVEFORMATEX) ? len : sizeof(WAVEFORMATEX));
            ...
            if (len > 0) {
                ...
                if (w->cbSize > 0) {
                    if (len < w->cbSize) {
                        ws_log(log::M_DEMUXER, log::ERR)
                            << "[demux_audio] truncated extradata ("
                            << len << " < "
                            << w->cbSize << ")"
                            << endlog;

                        stream_read(s,(char*)((char*)(w)+sizeof(WAVEFORMATEX)),len);
                        len = 0;
                    } else {
            ...
            if( ws_log::testLevel(log::M_DEMUXER, log::VERBOSE) ) print_wave_header(w, log::VERBOSE);
            if(len)
                stream_skip(s,len);
            do {
            ...
            if((w->wFormatTag == 0x01) && (w->nChannels == 2) && (w->nSamplesPerSec == 44100)) {
                ...
                stream_read(s, (char*)buf, sizeof(buf));
    ...
    if(stream_tell(s) != demuxer->movi_start) {
        ws_log::write(log::M_DEMUX, log::VERBOSE, "demux_audio: seeking from 0x%X to start pos 0x%X\n",
                (int)stream_tell(s), (int)demuxer->movi_start);
        stream_seek(s,demuxer->movi_start);
        if (stream_tell(s) != demuxer->movi_start) {
            ws_log::write(log::M_DEMUX, log::VERBOSE, "demux_audio: seeking failed, now at 0x%X!\n",
                    (int)stream_tell(s));
            if (next_frame_pos) {
                ws_log::write(log::M_DEMUX, log::VERBOSE, "demux_audio: seeking to 0x%X instead\n",
                        (int)next_frame_pos);
    ...
    ws_log::write(log::M_DEMUXER,log::VERBOSE, "demux_audio: audio data 0x%X - 0x%X  \n",(int)demuxer->movi_start,(int)demuxer->movi_end);
...
static int demux_audio_fill_buffer(demuxer_t *demuxer, demux_stream_t *ds)
{
    ...
    demux_packet_t* dp;
    sh_audio_t* sh_audio = (sh_audio_t*)ds->sh;
    demuxer_t* demux = ds->demuxer;
    da_priv_t* priv = (da_priv_t*)demux->priv;
    ...
    switch(priv->frmt) {
        case MP3 :
            while(1) {
                ...
                if (demux->movi_end && stream_tell(s) >= demux->movi_end)
                    ...
                    if (stream_read(s, (char*)dp->buffer + 4,len-4) != len-4) {
        ...
        case WAV : {
            ...
            len = sh_audio->wf->nAvgBytesPerSec;
            if (demux->movi_end && len > demux->movi_end - stream_tell(s)) {
                /* do not read beyond end, there might be junk after data chunk */
                len = demux->movi_end - stream_tell(s);
                if (len <= 0) return 0;
            }
            if (align)
                len = (len + align - 1) / align * align;
            dp = new_demux_packet(len);
            len = stream_read(s, (char*)dp->buffer,len);
            priv->next_pts += len/(double)sh_audio->i_bps;
            break;
        }
        ...
        default:
            ws_log::write(log::M_DEMUXER,log::WARN, MSGTR_DEMUX_AUDIO_UnknownFormat,priv->frmt);
            return 0;
    }
...
static void demux_audio_seek(demuxer_t *demuxer,float rel_seek_secs,float audio_delay,int flags)
{
    ...
    if(!(sh_audio = (sh_audio_t*)demuxer->audio->sh))
        return;
    s = demuxer->stream;
    priv = (da_priv_t*)demuxer->priv;

    if(priv->frmt == MP3 && PUBLIC_hr_mp3_seek && !(flags & 2)) {
