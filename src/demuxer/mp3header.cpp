/** @file
 * Openwsp - mpeg3 header operation.
 */
 ...
 */
int getmp3Header(unsigned char* hbuf,int* chans, int* srate, int* spf, int* mpa_layer, int* br){
...
//    printf("head=0x%08X\n",newhead);
	...
	ws_log(log::M_DEMUXER, log::DBG2) << "head_check failed" << endlog;
	return -1;
    }
#endif
    ...
    if(layer==4){
      ws_log(log::M_DEMUXER, log::DBG2) << "not layer-1/2/3" << endlog;
      return -1;
    }

    sampling_frequency = ((newhead>>10)&0x3);  // valid: 0..2
    if(sampling_frequency==3){
	ws_log(log::M_DEMUXER, log::DBG2) << "invalid sampling_frequency" << endlog;
	return -1;
    }
...
//      ws_log(MSGT_DEMUXER,MSGL_DBG2,"Free format not supported.\n");
//      return -1;
//    }
    ...
    ws_log::write(log::M_DEMUXER,log::DBG2, "FRAMESIZE: %d, layer: %d, bitrate: %d, mult: %d\n",
    	framesize, layer, tabsel_123[lsf][layer-1][bitrate_index], mult[layer-1]);
    if(!framesize){
	ws_log(log::M_DEMUXER, log::DBG2) << "invalid framesize/bitrate_index" << endlog;
	return -1;
    }
