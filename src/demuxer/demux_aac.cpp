...
static int demux_aac_init(demuxer_t *demuxer)
{
	...
	priv = (aac_priv_t*)calloc(1, sizeof(aac_priv_t));
	if(!priv)
		return 0;
...
static int demux_aac_probe(demuxer_t *demuxer) {
	...
	while(probed-init <= 32768 && cnt < 8) {
		...
		priv->buf[0] = 0xFF;
		if(stream_read(demuxer->stream, (char*)&(priv->buf[1]), 7) < 7)
			goto fail;
	...
	ws_log(log::M_DEMUX, log::VERBOSE)
	    << "demux_aac_probe, INIT: " << (uint64_t)init
	    << ", PROBED: " << (uint64_t)probed
	    << ", cnt: " << cnt
	    << endlog;
...
static int demux_aac_fill_buffer(demuxer_t *demuxer, demux_stream_t *ds)
{
	...
	while(! demuxer->stream->eof)
	{
		...
		if(stream_read(demuxer->stream, (char*)&(priv->buf[2]), 6) < 6)
			return 0;
		...
		if(len > 0)
		{
			...
			stream_read(demuxer->stream, (char*)&(dp->buffer[8]), len-8);
			if(srate)
...
static void demux_aac_seek(demuxer_t *demuxer, float rel_seek_secs, float audio_delay, int flags)
{
	...
	if(time > 0)
	{
		...
		while(nf > 0)
		{
			if(stream_read(demuxer->stream, (char*)priv->buf, 8) < 8)
				break;
			len = aac_parse_frame(priv->buf, &srate, &num);
			if(len <= 0)
			{
