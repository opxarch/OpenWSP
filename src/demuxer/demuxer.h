...
typedef struct demux_packet_st {
    ...
    int refcount;   /* refcounter for the master packet, if 0, buffer can be free() */
...
typedef struct demuxers_desc_st {
    ...
    int (*check_file)(struct demuxer_s *demuxer); /*< Mandatory if safe_check == 1, else optional*/
    /* Get packets from file, return 0 on eof*/
    int (*fill_buffer)(struct demuxer_s *demuxer, demux_stream_t *ds); /*< Mandatory*/
    /* Open the demuxer, return demuxer on success, NULL on failure*/
    struct demuxer_s* (*open)(struct demuxer_s *demuxer); /*< Optional*/
    /* Close the demuxer*/
    void (*close)(struct demuxer_s *demuxer); /*< Optional*/
    /* Seek*/
    void (*seek)(struct demuxer_s *demuxer, float rel_seek_secs, float audio_delay, int flags); /*< Optional*/
    /* Control*/
    int (*control)(struct demuxer_s *demuxer, int cmd, void *arg); /*< Optional*/
} demuxer_desc_t;
...
static inline void *realloc_struct(void *ptr, size_t nmemb, size_t nsize) {
    if (nmemb > SIZE_MAX / nsize) {
        free(ptr);
        return NULL;
    }
    return realloc(ptr, nmemb * nsize);
}
